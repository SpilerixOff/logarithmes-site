...
(function(){
  const bg=document.createElement('canvas');bg.className='bg';document.addEventListener('DOMContentLoaded',()=>document.body.prepend(bg));
  ...
  function draw(){const w=innerWidth,h=innerHeight;const g=ctx.createRadialGradient(w*.7,-h*.1,0,w*.7,-h*.1,Math.max(w,h));g.addColorStop(0,'#242f55aa');g.addColorStop(1,'#0000');ctx.clearRect(0,0,w,h);ctx.fillStyle=g;ctx.fillRect(0,0,w,h);
    glyphs.forEach(o=>{o.x+=o.vx;o.y+=o.vy;if(o.x<-60||o.x>w+60)o.vx*=-1;if(o.y<-60||o.y>h+60)o.vy*=-1;ctx.save();ctx.globalAlpha=o.a;ctx.fillStyle=o.t==='+'?'#b39bff':'#9a8cff';ctx.font=`800 ${o.s}px Inter`;ctx.textBaseline='top';ctx.fillText(o.t,o.x,o.y);ctx.restore();});requestAnimationFrame(draw);}
...
const T=[
  {title:'Règle PRODUIT', parts:['Formule : \\( \\log_a(xy)=\\log_a x+\\log_a y \\).','Astuce : multiplier **à l\'intérieur** → additionner **à l\'extérieur**.','Exemple : \\( \\log(200)=\\log(2\\cdot 100)=\\log2+\\log100=\\log2+2 \\).'], alt:'Multiplie les facteurs ⇒ additionne les logs.'},
...
let ti=0, td=0;
function openTutor(){ $('#tPanel').style.display='block'; ti=0; td=0; renderTutor(); }
function closeTutor(){ $('#tPanel').style.display='none'; }
function tMore(){ td=Math.min(td+1,2); renderTutor(); }
function tNext(){ if(ti<T.length-1){ ti++; td=0; renderTutor(); } else showTQuiz(); }
function tAlt(){ $('#tBox').innerHTML='<div class="step">'+T[ti].alt+'</div>'; mjx($('#tBox')); }
...
function showTQuiz(){ const Q=[{q:'\\(\\log(25)+\\log(4)=?\\)',a:'2'},{q:'\\(\\log(\\tfrac{20}{5})=?\\)',a:'\\log(4)'},{q:'\\(\\log(\\sqrt{10})=?\\)',a:'1/2'}]; const cur=Q[ti%Q.length];
  $('#tQuiz').innerHTML = `<div class="step"><b>Quiz :</b> ${cur.q}<div class="row" style="margin-top:6px"><input id="tAnswer" class="input" style="width:220px" placeholder="Ta réponse"><button id="btnVerify" class="btn safe">Vérifier</button></div><div id="tFb" class="small"></div></div>`;
  mjx($('#tQuiz')); $('#btnVerify').addEventListener('click', ()=>{ const v=($('#tAnswer').value||'').replace(/\s+/g,'').replace(',','.'); const s=cur.a.replace(/\s+/g,''); $('#tFb').innerHTML = (v===s)? '✔️ Correct' : '✖️ Réessaie : applique la règle.'; });
}
...
function gen(type){
  ...
  $('#qBox').innerHTML = ex.t; $('#qBox').dataset.ans = ex.ans; $('#qBox').dataset.hint = ex.hint; mjx($('#qBox')); $('#ans').value=''; $('#fb').textContent='';
}
...
function openCalc(){ $('#calcPanel').style.display='block'; $('#calcIn').focus(); }
function closeCalc(){ $('#calcPanel').style.display='none'; }
...
function bindAll(){
  const map=[['btnTutor',openTutor],['btnTutor2',openTutor],['btnCloseT',closeTutor],['btnMore',tMore],['btnNext',tNext],['btnAlt',tAlt],
             ['btnGenB',()=>gen('B')],['btnGenC',()=>gen('C')],['btnGenD',()=>gen('D')],['btnCheck',check],
             ['btnCalc',openCalc],['btnCalcClose',closeCalc]];
  map.forEach(([id,fn])=>{ const el=document.getElementById(id); if(el && !el.__bound){ el.addEventListener('click',fn); el.__bound=true; } });
  $$('.calc-key[data-ins]').forEach(el=>{ if(!el.__bound){ el.addEventListener('click',()=>calcInsert(el.dataset.ins)); el.__bound=true; } });
  const eq=document.getElementById('btnCalcEq'); if(eq && !eq.__bound){ eq.addEventListener('click',calcEval); eq.__bound=true; }
  const clr=document.getElementById('btnCalcClear'); if(clr && !clr.__bound){ clr.addEventListener('click',calcClear); clr.__bound=true; }
  const cpy=document.getElementById('btnCalcCopy'); if(cpy && !cpy.__bound){ cpy.addEventListener('click',calcCopy); cpy.__bound=true; }
}
document.addEventListener('DOMContentLoaded', bindAll); let tries=0; const int=setInterval(()=>{ tries++; bindAll(); if(tries>20) clearInterval(int); }, 200);
